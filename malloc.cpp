/*
  ...
  This version of malloc.c based on the versions listed below 
...
#define MLOCK_T std_mlock_t
#define CURRENT_THREAD        std::this_thread::get_id()

static FORCEINLINE int std_acquire_lock (MLOCK_T *sl) {
  sl->l.lock();
  ++sl->c;
  return 0;
}

static FORCEINLINE void std_release_lock (MLOCK_T *sl) {
  sl->l.unlock();
  --sl->c;
}

static FORCEINLINE int std_try_lock (MLOCK_T *sl) {
  if(sl->l.try_lock()){
    ++sl->c;
    return 1;
  }
...
static FORCEINLINE int std_init_lock (MLOCK_T *sl) {
  new (&sl->l) std::mutex();
  sl->c = 0;
  return 0;
}

static FORCEINLINE int std_islocked (MLOCK_T *sl) {
  if(!std_try_lock(sl)){
    int ret = (sl->c != 0);
    std_release_lock(sl);
    return ret;
...
#define INITIAL_LOCK(sl)      std_init_lock(sl)
#define ACQUIRE_LOCK(sl)      std_acquire_lock(sl)
#define RELEASE_LOCK(sl)      std_release_lock(sl)
#define TRY_LOCK(sl)          std_try_lock(sl)
#define IS_LOCKED(sl)         std_islocked(sl)
static MLOCK_T magic_init_mutex;
#if HAVE_MORECORE
static MLOCK_T morecore_mutex;
#endif /* HAVE_MORECORE */
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    char* mem = static_cast<char*>( internal_malloc(m, req) );
    if (mem != 0) {
      void* leader = 0;
      void* trailer = 0;
      mchunkptr p = mem2chunk(mem);
...
static void** ialloc(mstate m,
  ...
  */
  ...
  if (chunks != 0) {
    ...
    if (n_elements == 0)
      return static_cast<void**>( internal_malloc(m, 0) );
    marray = 0;
